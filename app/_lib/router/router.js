...
class Router {
  ...
  hrefTo(name, params, search) {
    ...
    let path = route.path;
    if (params) {
      path = path.replace(/:([^\/]+)/gi, (param, paramName) => {
        if (!params[paramName]) {
  ...
  isPathMatchesRouteOrParents(path) {
    ...
    if (currentRoute && currentRoute.route) {
      const paramsMatch = Object.keys(matchingRoute.params).every((key) => {
...
Router.prototype.isPathMatchesRouteOrParents = memoize(Router.prototype.isPathMatchesRouteOrParents, function (path) {
